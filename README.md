...
`nix/common.nix` を編集します。

- **パッケージ追加**: `home.packages` のリストに nixpkgs の名前を足す ([search.nixos.org](https://search.nixos.org/packages) で確認)
- **設定リンク追加**: `xdg.configFile` (→ `~/.config/<name>`) か `home.file` (→ ホーム直下) に1行足す

> **補足**: `nix/common.nix` の symlink 定義は `setup.sh` のリンク対象と別管理です。新しい設定を足すときは両方を意識してください (Nix側は `common.nix`、Nixなし側は `setup.sh` が自動スキャン)。
