...
### symlink を掃除する (`clean.sh`)

`setup.sh` が張ったリンクだけをまとめて削除します。消すのはリンクのみで、実体ファイルやリポジトリには触れません。検証用マシンを素の状態に戻したいときや、フォルダ名変更で残ったリンク切れを掃除したいときに使います。

```sh
./clean.sh --dry-run   # 消す対象を確認
./clean.sh             # 確認のうえ削除
```

「リンク先がこのリポジトリ配下を指す symlink」だけを対象にするため、リンク切れになっていても検出・削除できます。
